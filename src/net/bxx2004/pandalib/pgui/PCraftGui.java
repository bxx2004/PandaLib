...
public class PCraftGui extends CustomGui{
    ...
    private CustomItem by;
    private CustomItem result;
    private CustomItem[] craftitem;
    private Inventory craftgui;
    private CustomItem esc;
     ...
     */
    public PCraftGui(String title, CustomItem by, CustomItem result, CustomItem... craftitem){
        this.title = title;
        this.by = by;
        this.result = result;
     ...
     * @param by 合成方式
     */
    public void setBy(CustomItem by) {
        this.by = by;
        update();
    }
    /**
     ...
     * @return 返回合成材料
     */
    public CustomItem[] getCraftitem() {
        return craftitem;
    }
    /**
     * 设置合成材料
     ...
     */
    public void setCraftitem(CustomItem[] craftitem) {
        this.craftitem = craftitem;
        update();
    }
    /**
     ...
     * @param esc 材质
     */
    public void setESC(CustomItem esc) {
        this.esc = esc;
        update();
    }
    ...
    private void openAnimation(Player player, Animation animation, int speed){
        ...
        if (animation == Animation.BAR){
            new BukkitRunnable(){
                ...
                @Override
                public void run() {
                    craftgui.setItem(i,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    i= i+1;
                    if (i > 17){
                        craftgui.clear();
        ...
        if (animation == Animation.BRUSH){
            craftgui.setItem(0,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
            craftgui.setItem(1,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
            craftgui.setItem(9,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
            new BukkitRunnable(){
                int s1 = 2;
                int s2 = 10;
                int s3 = 18;
                ...
                public void run() {
                    try {
                        craftgui.setItem(s1,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        craftgui.setItem(s2,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        craftgui.setItem(s3,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        s1= s1+1;
                        s2= s2+1;
                        s3= s3+1;
                    }catch (Exception e){
            ...
            new BukkitRunnable(){
                ...
                public void run() {
                    if (who == 0 || who == 3 || who == 6){
                        craftgui.setItem(s3, new CustomItem(Material.AIR));
                        craftgui.setItem(s1,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    }
                    if (who == 1 || who == 4 || who == 7) {
                        craftgui.setItem(s1, new CustomItem(Material.AIR));
                        craftgui.setItem(s2,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    }
                    if (who == 2 || who == 5 || who == 8){
                        craftgui.setItem(s2, new CustomItem(Material.AIR));
                        craftgui.setItem(s3,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    }
                    who = who + 1;
                    if (who >= 9){
