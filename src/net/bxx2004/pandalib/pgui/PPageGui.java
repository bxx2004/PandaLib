package net.bxx2004.pandalib.pgui;
...
public class PPageGui extends CustomGui{
    ...
    }.hook(PandaLib.getInstance());
    private Inventory pagegui;
    private CustomItem tip;
    private CustomItem barrier;
    private CustomItem back;
    private CustomItem next;
    private String title;
    private int page;
    private List<CustomItem> items;
    private HashMap<Integer, List<CustomItem>> pageitem;
    private int speed;
     ...
     * @param next 下一页材质
     */
    public PPageGui(String title, CustomItem tip, CustomItem barrier, CustomItem back, CustomItem next){
        this.pageitem = new HashMap<Integer, List<CustomItem>>();
     ...
     * @param page 页面
     */
    public List<CustomItem> getItemOfPage(int page){
     ...
     * @return 返回顶部提示物品
     */
    public CustomItem getTip() {
        return tip;
    }
    /**
     * @param tip 设置顶部提示物品
     */
    public void setTip(CustomItem tip) {
        this.tip = tip;
        update();
    }
    /**
     * @return 返回边框材质
     */
    public CustomItem getBarrier() {
        return barrier;
    }
    /**
     * @param barrier 设置边框材质
     */
    public void setBarrier(CustomItem barrier) {
        this.barrier = barrier;
        update();
    }
    /**
     * @return 返回上一页材质
     */
    public CustomItem getBack() {
        return back;
    }
    /**
     * @param back 设置上一页材质
     */
    public void setBack(CustomItem back) {
        this.back = back;
        update();
    }
    /**
     * @return 返回下一页材质
     */
    public CustomItem getNext() {
        return next;
    }
    /**
     * @param next 设置下一页材质
     */
    public void setNext(CustomItem next) {
        this.next = next;
        update();
    }
    ...
    private void openAnimation(Player player, Animation animation, int speed){
        ...
        if (animation == Animation.BAR){
            new BukkitRunnable(){
                ...
                @Override
                public void run() {
                    pagegui.setItem(i,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    i= i+1;
                    if (i > 26){
                        update();
                        if (items == null){
        ...
        if (animation == Animation.BRUSH){
            pagegui.setItem(9,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
            pagegui.setItem(27,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
            pagegui.setItem(45,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
            new BukkitRunnable(){
                int s1 = 0;
                int s2 = 10;
                int s3 = 18;
                ...
                public void run() {
                    try {
                        pagegui.setItem(s1,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        pagegui.setItem(s2,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        pagegui.setItem(s3,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        pagegui.setItem(s4,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        pagegui.setItem(s5,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        pagegui.setItem(s6,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                        s1= s1+1;
                        s2= s2+1;
                        s3= s3+1;
                        s4 = s4+1;
            ...
            new BukkitRunnable(){
                ...
                public void run() {
                    if (who == 0 || who == 3 || who == 6){
                        pagegui.setItem(s3, new CustomItem(Material.AIR));
                        pagegui.setItem(s1,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    }
                    if (who == 1 || who == 4 || who == 7) {
                        pagegui.setItem(s1, new CustomItem(Material.AIR));
                        pagegui.setItem(s2,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    }
                    if (who == 2 || who == 5 || who == 8){
                        pagegui.setItem(s2, new CustomItem(Material.AIR));
                        pagegui.setItem(s3,new CustomItem(Material.GREEN_STAINED_GLASS_PANE,"&e加载中···"));
                    }
                    who = who + 1;
                    if (who >= 9){
